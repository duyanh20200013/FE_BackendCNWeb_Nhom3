...
import { getAllCreateHouse, confirmCreateHouse, rejectCreateHouse } from '@/api/houseApi';
...
function Admin() {
    ...
    const handleOnClick1 = async (id) => {
        try {
            await confirmCreateHouse({ houseId: id });
            navigate(0);
        } catch (err) {
            console.log(err)
        }
    ...
    const handleOnClick2 = async (id) => {
        try {
            await rejectCreateHouse({ houseId: id });
            navigate(0);
        } catch (err) {
            console.log(err)
        }
    ...
    useEffect(() => {
        async function getContractList() {
            try {
                const res = await getAllCreateHouse();
                ...
                <Container>
                    ...
                    <div className='flex justify-between'>
                        <div className='w-[30%] flex flex-col gap-2'>
                            ...
                            <Button label="Nhà được tạo"></Button>
